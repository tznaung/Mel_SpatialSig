...
final_coeffs = read.csv("final_coeffs_S100B.csv")
...
final_coeffs_2 = final_coeffs[c(2,3,7,10,14),]
...
for(i in 1:dim(train_data3)[1]){
  train_data3$lasso_scores[i] = sum(as.matrix(train_data3[i,1:nGene_2])*(final_coeffs$s0[2:dim(final_coeffs_2)[1]]))
} 
...
pheatmap(train_data5,annotation_row = train_data6, main = "Signature genes CD68 Comp",
...
A = ggsurvplot(fit, data = train_data3,
           title = "CD68 compartment w/ S100B signatures",
...
A$plot <- A$plot+ 
  ggplot2::annotate("text", 
                    ...
                    label = "HR = 2.3 (0.95-5.8) \n p = 0.064", size = 4)
A

# CD68 Compartment with S100B Signatures
