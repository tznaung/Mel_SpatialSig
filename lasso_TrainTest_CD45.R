...
# dex_df = read.csv("CD45_train_set_Sig_Genes.csv")
...
pheatmap(test_data2,annotation_row = test_data3, main = "Signature genes CD45 Comp",
         cluster_rows = T, color = colorRampPalette(rev(brewer.pal(n = 11, name ="RdYlGn")))(15),
         # cutree_cols = 2,
         cutree_rows = 2, 
...
CD45_2$lasso_scores = matrix(0,dim(CD45_2)[1],1)
for(i in 1:dim(CD45_2)[1]){
  CD45_2$lasso_scores[i] = sum(as.matrix(CD45_2[i,nonzero])*t(coeffs[2:dim(coeffs)[1],]))
}
...
pROC_obj <- roc(CD45_2$response[test_idx],CD45_2$lasso_scores[test_idx],
                smoothed = TRUE,
                # arguments for ci
