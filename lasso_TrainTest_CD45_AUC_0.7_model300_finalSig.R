...
CD45_df = read.csv("CD45_av.csv", header = T) # quantile normalized average counts from 2 cores for each patient
CD45_df$SpotID <- CD45_df$X # core representing patient ID
...
# read the AUC result file
aucs = read.csv("aucs_CD45_300.csv")
...
for (cSplit in good_models){
  ...
  train_data = CD45_2[train_idx,]
  train_data2 = train_data[,3:3738]
  test_data = CD45_2[test_idx,]
  test_data2 = test_data[,3:3738]
  ...
  write.csv(response$table, "CD45_train_set_Sig_Genes.csv")
...
write.csv(goodModelGeneCounts,"goodModelCounts_CD45_from300.csv")
#goodModelGeneCounts = read.csv("goodModelCounts_CD45.csv",row.names = 1)
...
final_genes = names(goodModelGeneProps)[which(goodModelGeneProps>0.1)]
...
model_cv <- glmnet(as.matrix(train_data3[,1:length(final_genes)]), train_data3$response, alpha = 1, lambda = 0, standardize = FALSE, family = "binomial")
